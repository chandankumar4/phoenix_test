require Logger;
require IEx;
defmodule OpenebsciDashnoardApi.Scheduler do
  use Quantum.Scheduler, otp_app: :openebsci_dashnoard_api
  use EctoConditionals, repo: OpenebsciDashnoardApi.Repo

	def save_project_names do
		projects = RubyElixir.GitLabProxy.get_gitlab_project_names
		Enum.map(projects, fn(source_project) ->

			prec = %OpenebsciDashnoardApi.Projects{name: source_project}
			OpenebsciDashnoardApi.Repo.insert(prec)
		end)
  end
    ...
    {skp_found, skp_record} = %OpenebsciDashnoardApi.SourceKeyProjects{source_id: Integer.to_string(source_project["id"])} |> find_by(:source_id)
    ...
    {sp_found, sp_record} = %OpenebsciDashnoardApi.Projects{id: (skp_record.new_id || -1)} |> find_by(:id)
    case sp_found do
      :not_found ->
        Logger.info fn ->
          "upsert: sp_found not found"
        end
        sp_record = %OpenebsciDashnoardApi.Projects{}
      _ -> :ok
    end
    ...
    changeset = OpenebsciDashnoardApi.Projects.changeset(sp_record, %{name: source_project["name"]})
    ...
    case sp_found do
      :found ->
        {_, sp_record} = OpenebsciDashnoardApi.Repo.update(changeset)
      :not_found ->
        {_, sp_record} = OpenebsciDashnoardApi.Repo.insert(changeset)
    end
    ...
    changeset = OpenebsciDashnoardApi.SourceKeyProjects.changeset(skp_record, %{new_id: sp_record.id})
    case skp_found do
      :found ->
        {_, skp_record} = OpenebsciDashnoardApi.Repo.update(changeset)
      :not_found ->
        {_, skp_record} = OpenebsciDashnoardApi.Repo.insert(changeset)
    end
    end)
